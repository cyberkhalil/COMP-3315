...
(function () {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const yearSpans = document.querySelectorAll(
      "#current-year, #lecture-year, .copyright-year span",
    );
    ...
    lectureBtns.forEach((btn) => {
      btn.addEventListener("click", function (e) {
        const targetChapter =
          this.closest(".week-card")?.querySelector(".chapter-title-main")
            ?.innerText || "Lecture";
        console.log(
          `Navigating to ${targetChapter} material: ${this.getAttribute("href")}`,
        );
      });
    });
